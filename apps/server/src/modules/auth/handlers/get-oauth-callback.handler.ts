...
export const getOAuthCallbackRoute = createRoute({
  ...
  description:
    "Handles the OAuth callback from the provider and exchanges code for tokens",
  request: {
    params: z.object({
      provider: z
    ...
    query: z.object({
      ...
      error_description: z
        ...
        .openapi({
          description:
            "Error description returned by the OAuth provider, if any",
      ...
      state: z
        ...
        .openapi({
          description:
            "The state parameter returned by the OAuth provider for CSRF protection",
...
export const getOauthCallbackHandler: AppRouteHandler<
